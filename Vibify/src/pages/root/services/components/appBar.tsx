...
function LinkElement({ item }: { item: LinkItem }) {
    ...
    return (
        <Link
            key={item.title}
            to={item.url}
            className={`${isArabic ? "sm:last:ml-2" : "sm:last:mr-2"} flex items-center ${isArabic ? "flex-row-reverse" : ""} justify-center rounded-md p-2 text-sm font-medium transition-colors relative group text-gray-500 hover:text-gray-900 hover:bg-gray-100  focus:text-primary ${isActive ? "text-primary" : "text-muted-foreground hover:text-primary focus:text-primary"}`}
        >
            <item.icon className={`size-5 ${isActive && item.title === "Notifications" && "fill-primary"}`} />
            <span className="sr-only sm:not-sr-only sm:ml-2 sm:text-sm">{t(item.title)}</span>
