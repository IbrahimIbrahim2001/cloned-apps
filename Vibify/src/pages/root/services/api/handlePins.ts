...
export const addToPins = async (track: Track | DatabaseTrack) => {
    try {
        const {
            data: { user },
        ...
        console.error("Error adding/updating pins:", error)
    }
}


export const removeFromPins = async (track: Track | DatabaseTrack) => {
    try {
        const {
            data: { user },
        } = await supabase.auth.getUser()
        ...
        const { error } = await supabase.from("Pins").delete().eq("user_id", user.id).eq("track_id", trackId)
